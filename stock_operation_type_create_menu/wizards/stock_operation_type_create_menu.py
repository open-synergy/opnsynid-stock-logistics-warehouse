...
class StockOperationTypeCreateMenu(models.TransientModel):
    ...
    def default_get(self, fields):
        res = super(StockOperationTypeCreateMenu, self).default_get(fields)
    ...
    def _create_warehouse_menu(self, picking_type):
        ...
        res = {"name": warehouse_name, "sequence": 100, "parent_id": parent_menu_id.id}

        warehouse_menu_id = obj_ir_ui_menu.create(res)

        picking_type.warehouse_id.menu_id = warehouse_menu_id.id
        ...
        if not picking_type.menu_id:
            if not picking_type.warehouse_id:
                ...
                if not picking_type.warehouse_id.menu_id:
                    parent_id = self._create_warehouse_menu(picking_type)
                else:
    ...
    def button_create_menu(self):
        ...
        window_action_id = self._create_window_action(picking_type)

        self._create_menu(picking_type, window_action_id)
...
class StockOperationTypeCreateMenuViewDetail(models.TransientModel):
    ...
    wizard_id = fields.Many2one(
        string="Wizard", comodel_name="stock.operation_type_create_menu"
    )
