...
class TestCreateResetMenu(TestBaseTypeCreateMenu):
    def test_create_reset_menu(self):
        ...
        # Reset Menu
        if self.picking_type.menu_id.id or self.picking_type.window_action_id:
        ...
        self.assertEqual(False, self.picking_type.menu_id.id)
        self.assertEqual(False, self.picking_type.window_action_id.id)

        new = self.wiz.with_context(
            active_model="stock.picking.type", active_ids=[self.picking_type.id]
        ).new()
