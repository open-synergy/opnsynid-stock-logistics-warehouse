...
class StockWarehouse(models.Model):
    ...
    employee_in_type_id = fields.Many2one(
        string="Employee Equipment In Type", comodel_name="stock.picking.type"
    )

    employee_out_type_id = fields.Many2one(
        string="Employee Equipment Out Type", comodel_name="stock.picking.type"
    )

    employee_in_route_id = fields.Many2one(
        string="Employee Equipment In Route", comodel_name="stock.location.route"
    )

    employee_out_route_id = fields.Many2one(
        string="Employee Equipment Out Route", comodel_name="stock.location.route"
    )
    ...
    def _get_employee_in_src_location(self):
        ...
        employee_loc = self.env["ir.property"].get(
            "property_stock_employee_id", "res.partner"
        )
        return {
    ...
    def _prepare_employee_in_type(self):
        ...
        sequence = obj_sequence.create(self._prepare_employee_in_sequence())
    ...
    def _prepare_employee_out_type(self):
        ...
        sequence = obj_sequence.create(self._prepare_employee_out_sequence())
    ...
    def _create_employee_in_type(self):
        ...
        pick_type = obj_type.create(self._prepare_employee_in_type())
        return pick_type
    ...
    def _create_employee_out_type(self):
        self.ensure_one()
        obj_type = self.env["stock.picking.type"]
        pick_type = obj_type.create(self._prepare_employee_out_type())
        return pick_type
    ...
    def create(self, values):
        ...
        employee_in_route = new_wh._create_route_employee_in()
        employee_out_route = new_wh._create_route_employee_out()
        new_wh.write(
            {
                "employee_in_route_id": employee_in_route.id,
                "employee_out_route_id": employee_out_route.id,
                "route_ids": [(4, employee_in_route.id), (4, employee_out_route.id)],
            }
        )
        return new_wh
    ...
    def change_route(
        ...
        if new_reception_step:
            src_loc = self._get_employee_in_src_location()[new_reception_step]
            dest_loc = self._get_employee_in_dest_location()[new_reception_step]
            res_supp = {
                "default_location_src_id": src_loc.id,
                "allowed_location_ids": [(6, 0, [src_loc.id])],
                "default_location_dest_id": dest_loc.id,
        ...
        if new_delivery_step:
            src_loc = self._get_employee_out_src_location()[new_delivery_step]
            dest_loc = self._get_employee_out_dest_location()[new_delivery_step]
            res_cust = {
                "default_location_src_id": src_loc.id,
                "allowed_location_ids": [(6, 0, [src_loc.id])],
                "default_location_dest_id": dest_loc.id,
    ...
    def _create_route_employee_in(self):
        ...
        return obj_route.create(self._prepare_route_employee_in())

    @api.multi
    def _create_route_employee_out(self):
        self.ensure_one()
        obj_route = self.env["stock.location.route"]
        return obj_route.create(self._prepare_route_employee_out())
