...
class StockInventoryRevaluation(models.Model):
    ...
    def _check_qty_available(self):
        ...
        for variant in self.product_template_id.product_variant_ids:
            if variant.qty_available <= 0.0:
                raise UserError(
                    _(
                        "Cannot do an inventory value change if the "
                        "quantity available for product %s "
                        "is 0 or negative" % variant.name
                    )
                )
    ...
    def _check_negative(self):
        ...
        if self.revaluation_type == "inventory_value":
            if self.new_value < 0:
                raise UserError(
                    _(
                        "The new value for product %s cannot "
                        "be negative" % self.product_template_id.name
                    )
                )
    ...
    def _generate_accounting_entry(self):
        ...
        for prod_variant in self.product_template_id.product_variant_ids:
            ...
            if qty:
                debit_account_id, credit_account_id = self._get_account(amount_diff)
    ...
    def _get_avg_std_amount_diff(self, product_variant):
        ...
        if self.revaluation_type == "price_change":
            ...
            proportion = (
                product_variant.qty_available / self.product_template_id.qty_available
            )
