...
class StockWarehouse(models.Model):
    ...
    def _get_donation_out_dest_location(self):
        self.ensure_one()
        donation_in_loc = self.env["ir.property"].get(
            "property_stock_donation_out_id",
            "res.partner")
        return {
    ...
    def _create_donation_in_type(self):
        ...
        pick_type = obj_type.create(
            self._prepare_donation_in_type())
        return pick_type
    ...
    def _create_donation_out_type(self):
        ...
        obj_type = self.env["stock.picking.type"]
        pick_type = obj_type.create(
            self._prepare_donation_out_type())
        return pick_type
    ...
    def create(self, values):
        ...
        donation_in_route =\
            new_wh._create_route_donation_in()
        donation_out_route =\
            new_wh._create_route_donation_out()
        new_wh.write({
            "donation_in_route_id": donation_in_route.id,
            "donation_out_route_id": donation_out_route.id,
            "route_ids": [
                (4, donation_in_route.id),
                (4, donation_out_route.id)
            ],
        })
        return new_wh
    ...
    def change_route(
        ...
        if new_reception_step:
            src_loc = self._get_donation_in_src_location()[
                new_reception_step]
            dest_loc = self._get_donation_in_dest_location()[
                new_reception_step]
            res_supp = {
                "default_location_src_id": src_loc.id,
                "allowed_location_ids": [(6, 0, [src_loc.id])],
        ...
        if new_delivery_step:
            src_loc = self._get_donation_out_src_location()[
                new_delivery_step]
            dest_loc = self._get_donation_out_dest_location()[
                new_delivery_step]
            res_cust = {
                "default_location_src_id": src_loc.id,
                "allowed_location_ids": [(6, 0, [src_loc.id])],
    ...
    def _create_route_donation_in(self):
        ...
        return obj_route.create(
            self._prepare_route_donation_in())
    ...
    def _create_route_donation_out(self):
        ...
        obj_route = self.env["stock.location.route"]
        return obj_route.create(
            self._prepare_route_donation_out())
