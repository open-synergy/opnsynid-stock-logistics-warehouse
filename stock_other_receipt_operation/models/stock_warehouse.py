...
class StockWarehouse(models.Model):
    ...
    other_receipt_type_id = fields.Many2one(
        string="Other Receipt Type",
        comodel_name="stock.picking.type"
    )

    other_receipt_route_id = fields.Many2one(
        string="Other Receipt Route",
    ...
    def _prepare_other_receipt_type(self):
        ...
        sequence = obj_sequence.create(
            self._prepare_other_receipt_sequence())
    ...
    def _create_other_receipt_type(self):
        ...
        pick_type = obj_type.create(
            self._prepare_other_receipt_type())
        return pick_type
    ...
    def change_route(
        ...
        if new_reception_step:
            src_loc = self._get_other_receipt_src_location()[
                new_reception_step]
            dest_loc = self._get_other_receipt_dest_location()[
                new_reception_step]
            res_supp = {
                "default_location_src_id": src_loc.id,
                "allowed_location_ids": [(6, 0, [src_loc.id])],
    ...
    def _create_route_other_receipt(self):
        ...
        return obj_route.create(
            self._prepare_route_other_receipt())
